...
class PhilFisherCryptoAgent:
    ...
    def consult_crypto(self, symbol, timeframe, model_used="rule-based"):
        ...
        if rsi > 65 and latest_close > avg_close:
            signal = "BULLISH"
            action = "LONG"
            confidence = 85
            reasoning = "Strong growth and quality, invest after deep research."
        ...
        elif rsi < 35 and latest_close < avg_close:
            signal = "BEARISH"
            action = "SHORT"
            confidence = 75
            reasoning = "Growth slowing, consider selling."
            ...
            action = "HOLD"
            confidence = 60
            reasoning = "No clear quality growth opportunity."
        ...
        return {
            ...
            "discord_message": (
                f"**{symbol} ({timeframe})**\n"
                f"**Technical Summary:**\n"
                f"`Price: {format_price(latest_close)} | 60-MA: {format_price(avg_close)} | RSI(14): {rsi:.2f} | MACD: {macd:.2f}`\n"
                f"**Signal:** {signal}\n"
